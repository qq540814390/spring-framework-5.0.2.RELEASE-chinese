...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		...
		//解析HandlerMapping （url和method的关联关系）
		initHandlerMappings(context);
		//适配器（匹配的过程）
		initHandlerAdapters(context);
		//异常解析，处理@ExceptionHandler
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				//@EnableWebMvc初始化会注入三个HandlerAdapter（RequestMappingHandlerAdapter，HttpRequestHandlerAdapter，SimpleControllerHandlerAdapter）
				//一般情况下都是RequestMappingHandlerAdapter处理，SimpleControllerHandlerAdapter处理Controller接口的handler
				//适配器的目的是将方法的返回结果转换为ModelAndView
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			//JSP使用InternalResourceViewResolver，通常情况这里先调用ContentNegotiatingViewResolver（内容协商视图解析器）
