...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	//在这里创建proxy对象，这是InstantiationAwareBeanPostProcessor接口的方法
	@Override
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) throws BeansException {
		Object cacheKey = getCacheKey(beanClass, beanName);

		if (!StringUtils.hasLength(beanName) || !this.targetSourcedBeans.contains(beanName)) {
			//判断是否增强过，advisedBeans会保存所以增强的bean
			if (this.advisedBeans.containsKey(cacheKey)) {
				return null;
			}
			//isInfrastructureClass判断是不是aop基础的bean，如Advice，Pointcut，Advisor
			//shouldSkip判断是否需要跳过，这里走子类的方法，判断每个Advisor是否是AspectJPointcutAdvisor
		...
		//如果用户自定义了TargetSource，走下面逻辑，这里是个扩展点
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		if (specificInterceptors != DO_NOT_PROXY) {
			//保存当前bean在advisedBeans中，说明要进行增强了
